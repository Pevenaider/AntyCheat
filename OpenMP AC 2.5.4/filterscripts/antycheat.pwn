...
public OnClientCheckResponse(playerid, actionid, memaddr, retndata)
{
    switch(actionid)
    {
        ...
        case 0x45:
        {
        	if ( memaddr == AC_Player[playerid][addChecks][0] && retndata != 128 ) { AC_Player[playerid][pCheat][6] = 7; }
         	if ( memaddr == AC_Player[playerid][addChecks][1] && retndata != 192 ) { cheatDetected(playerid, "Modded client", 0); }
...
public OnClientCheckResponseHandler(playerid, BitStream:bs)
{
	...
	if ( actionid == 0x45 && memaddr == AC_Player[playerid][checkSampAddr] )
	{
		...
		{
			cheatDetected(playerid, "[RakNet] RakNet serialization anomaly", 2);
			return 0;
		}
...
public checkPlayer(playerid)
{
	...
	for (new i = 0; i < MAX_CHEATS; i++)
	{
    	switch ( AC_Player[playerid][pCheat][i] )
    	{
			...
			case 18:cheatDetected(playerid, "[3] S0beit / RakNet anomaly", 0);
		}
	}
    return 1;
}
